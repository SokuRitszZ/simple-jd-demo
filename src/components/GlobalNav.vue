...
function getSubOptions() {
  return new Array(Math.random() * 20 >>> 0).fill(0).map(() => faker.lorem.word(4))
}
  ...
  <nav id="home-nav" class="min-w-[1200px]">
    <div class="bg-[#E3E4E5]">
      <div class="w-[1200px] m-auto flex justify-between text-sm">
        ...
        <div id="header-options" class="text-sm text-[#999999] py-1 flex items-center">
          ...
          <div class="nav-item relative" :key="o" v-for="o of options">
            ...
            <div class="dropdown absolute hidden w-[300px] grid-cols-3">
              <span :key="oo" v-for="oo in getSubOptions()" class="sub-option">{{ oo }}</span>
            </div>
          </div>
