  ...
  <div id="main-grid" class="">
    ...
    <div id="other" style="grid-area: o" class="bg-white pt-5 text-[10px]">
      <div class="flex justify-center items-center gap-2 pb-3">
        ...
        <div class="">
          <div>Hi~欢迎逛京东!</div>
          <a href="#" class="hover:text-[#c81623]">登录</a> |
          <a href="#" class="hover:text-[#c81623]">注册</a>
        </div>
      </div>
      <div class="grid grid-cols-2 text-sm pb-2">
        <button
          class="w-fit m-auto px-3 py-1 text-white bg-[#e1251b] rounded-full"
        >
          新人福利
        </button>
        <button
          class="w-fit m-auto px-3 py-1 text-[#e5d790] bg-[#363634] rounded-full"
        >
          PLUS会员
        </button>
      </div>
      ...
      <div
        :key="n.id"
        v-for="n of news"
        class="flex items-center gap-1 px-2 mt-1"
      >
        <div class="bg-[#e1251b14] text-[#e1251b] w-[30px] text-center">
          {{ n.type }}
        </div>
        <a
          href="#"
          class="overflow-hidden text-ellipsis whitespace-nowrap w-[150px] text-[#666666] hover:text-[#c81623]"
          >{{ n.title }}</a
        >
      </div>
      <div class="pb-2"></div>
      ...
      <div class="grid grid-cols-3 grid-rows-4">
        <a href="#" :key="s.title" v-for="s in services">
          ...
          <div class="text-center hover:text-[#c81623] transition">
            {{ s.title }}
          </div>
        </a>
      </div>
