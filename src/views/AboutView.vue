...
const src = ref<string>('https://sdfsdf.dev/1000x1000.png,123456,123123');
...
function move(e: MouseEvent) {
  ...
  $canvas.value?.style.setProperty(
    'transform',
    `translate(${left}px, ${top}px)`
  );
  $imgBig.value?.style.setProperty(
    'transform',
    `translate(-${left * 4}px, -${top * 4}px)`
  );
}
...
<template>
  <div class="w-full h-screen flex justify-center items-center">
    <div class="relative w-[300px] h-[300px]">
      ...
      <div
        v-show="shouldShown"
        class="absolute top-0 left-full w-[400px] h-[400px] bg-black overflow-hidden"
      >
        <img
          ref="$imgBig"
          class="w-[1200px] h-[1200px] min-w-[1200px] min-h-[1200px] absolute top-0 left-0"
          :src="src"
          alt=""
        />
      </div>
